...
`scripts/execute.py`가 phase 안의 step을 순차 실행하는 하네스다. step 하나당 `codex exec` 세션 하나를 띄운다.

```
python3 scripts/execute.py <phase-dir> [--push]   # 예: python3 scripts/execute.py 0-mvp
```

- **step 실행** — `codex exec --dangerously-bypass-approvals-and-sandbox --dangerously-bypass-hook-trust <prompt>`를 저장소 루트에서 돌린다. 무인 실행이라 승인 프롬프트와 샌드박스를 끄고, `.codex/hooks.json` 훅도 신뢰 등록 없이 발동시킨다.
- **가드레일 주입** — 이 `AGENTS.md`와 `docs/*.md`를 매 step 프롬프트 앞에 붙인다. `codex exec`가 `AGENTS.md`를 자동으로 읽기도 하므로 규칙은 두 경로로 들어간다.
- **상태 파일** — 진행 상태는 `phases/<phase>/index.json`에 있다. step 세션이 자기 step의 `status`를 `completed`(+`summary`) / `error`(+`error_message`) / `blocked`(+`blocked_reason`)로 갱신하고, 타임스탬프와 커밋은 `execute.py`가 찍는다. 실패 시 최대 3회 재시도하며 직전 에러를 프롬프트에 넣어 준다.
- **세션 로그** — 각 step의 stdout/stderr는 `phases/<phase>/step<N>-output.json`에 남는다. 위의 시크릿·거래 내용 로깅 금지 규칙이 여기에도 적용된다.

`.codex/hooks.json`이 훅을 물린다.

- `PreToolUse[apply_patch]` → `scripts/hooks/tdd-guard.sh`. 테스트 파일이 없는 소스 파일을 만들거나 고치려 하면 차단한다. 패치 envelope(`*** Add File:` / `*** Update File:` / `*** Move to:`)에서 경로를 뽑아 검사한다.
- `PreToolUse[Bash]` → `rm -rf`, `git push --force`, `git reset --hard`, `DROP TABLE` 차단.
- `Stop` → `npm run lint && npm run build && npm run test`.

## 사람이 해야 하는 일 (에이전트가 할 수 없음)
