...
1. **컬럼 매핑 추론** — CSV 헤더와 **최대 20개 대표 행**만 보내 step 2의 `ColumnMapping`을 추론한다. 결과는 step 3 파서에 주입한다. **추론 결과를 그대로 믿지 마라** — 코드가 전체 행에 적용해 날짜·금액 95% 이상 파싱과 소비 행 부호 의미의 일관성을 검증하고, 미달이면 `column_mapping_failed`로 처리한다(ADR-012).
...
3. **해석 (1회 통합 호출)** — step 4가 만든 `recent12m`·`full` **두 scope의 집계 통계만** 입력으로 보내 각 scope의 AI 요약 + 절약 인사이트 + 후보 설명을 한 번에 생성한다. **거래 원본 전체를 보내지 마라.**
...
- **출력 검증**: 모든 응답을 Structured Outputs와 Zod 양쪽으로 검증하고, 통과하지 못하면 `analysis_failed`로 처리하라. 검증되지 않은 값을 DB나 UI로 흘려보내지 마라.
